...
def collate_torchsparse(voxel_size, use_coords, list_data):
    ...
    for group_pc in batch['points']:
        ...
        batch['batch_n_points'].append(coordinates.shape[0])
        batch['indexes'].append(torch.from_numpy(indexes))
        batch['inv_indexes'].append(torch.from_numpy(inv_indexes) + offset)
        if use_coords:
            batch['voxels'].append(torch.from_numpy(group_pc[indexes]))
        else:
            batch['voxels'].append(torch.from_numpy(group_pc[indexes, 3:]))
        batch_id += 1
    ...
    # batch['pc_overlap_mask'] = [torch.from_numpy(mask) for mask in batch['pc_overlap_mask']]
    # batch['pc_overlap_mask'] = torch.cat(batch['pc_overlap_mask'])
